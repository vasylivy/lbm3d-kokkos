...
void update(DistributionField fB, DistributionField fA, ScalarField u, ScalarField v, ScalarField w, ScalarField rho, Params &params, const int step, int &converged);
...
{				
...

	{
		...
		printf("Solving lid driven cavity (Re = %.2e, tau = %.2e, [%i x %i x %i] , RAM (MB) = %.1f...\n", params.re, params.tau, nx - 2, ny - 2, nz - 2, double(nx) * double(ny) * double(nz) * 8. * (2. * 19. + 3.) / (1024. * 1024.));
...
void update(DistributionField fB, 
	   ...
	   )
{
	...
	if ((step + 1) % params.output_rate == 0){
	    ...
	    Kokkos::parallel_reduce("check_if_steady_state", range_3d({1,1,1}, {D1-1,D2-1,D3-1}), is_steady_state(fA, u, v, w, rho, params.tol), Kokkos::BAnd<int>(converged));
	}
}
