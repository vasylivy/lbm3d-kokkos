...
struct collide_stream {
    ...
    collide_stream(DistributionField fB, DistributionField fA, Params params_) : fB(fB), fA(fA), params(params_)
    {
    }
...
struct bb_left {
    ...
    bb_left(DistributionField fB) : fB(fB)
    {
    }
...
struct bb_right {
    ...
    bb_right(DistributionField fB) : fB(fB), j(fB.extent(1) - 2)
    {
    }
...
struct bb_bottom{
    ...
    bb_bottom(DistributionField fB) : fB(fB)
    {
    }
...
struct bb_top{
    ...
    bb_top(DistributionField fB, Params params_) : fB(fB), u(params_.lid_u), w(params_.lid_w), i(fB.extent(0) - 2)
    {
    }
...
struct bb_front{
    ...
    bb_front(DistributionField fB) : fB(fB), k(fB.extent(2) - 2)
    {
    }
...
struct bb_back{
    ...
    bb_back(DistributionField fB) : fB(fB)
    {
    }
...
struct compute_macroscopic{
    ...
    compute_macroscopic(DistributionField f, ScalarField u, ScalarField v, ScalarField w, ScalarField rho): f(f), u(u), v(v), w(w), rho(rho)
    {
    }
...
struct load_state{
    ...
    Double weight[19] = { 1. / 3., 1./18, 1./18. , 1./18. , 1./18., 1./18.,  1./18. , 1./36., 1./36.,  1./36., 1./36., 1./36., 1./36.,  1./36., 1./36., 1./36., 1./36.,  1./36., 1./36.};
	
    load_state(DistributionField fA, DistributionField fB, ScalarField u, ScalarField v, ScalarField w, ScalarField rho) : fA(fA), fB(fB), u(u), v(v), w(w), rho(rho)
    {
    }
...
struct is_steady_state{
  ...
  void operator()(const int i, const int j, const int k, int &converged) const
  {
    ...
    converged = (fabs(umom_B - umom_A) < (tol * fabs(umom_A))) && (fabs(vmom_B - vmom_A) < (tol * fabs(vmom_A))) && (fabs(wmom_B - wmom_A) < (tol * fabs(wmom_A)));
  }
};
